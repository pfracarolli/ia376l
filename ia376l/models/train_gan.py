...
def train(args):
    ...
    for epoch in range(NUM_EPOCHS):
        for i, (imgs, _) in enumerate(dataloader, 0):
            ...
            real_128 = F.interpolate(
                real_cpu, size=(128, 128), mode='bilinear', align_corners=False
            )
            ...
            errD_fake = F.relu(
                torch.rand_like(output) * 0.2 + 0.8 + output
            ).mean()
            errD_fake.backward()
            ...
            if iters % (save_interval) == 0:
                ...
                vutils.save_image(
                    fake.detach(),
                    '%s/fake_samples_iter_%03d.png' % (save_path, iters),
                    nrow=4,
                    normalize=True,
                )
                vutils.save_image(
                    torch.cat([real_128, rec_all, rec_part]),
                    '%s/real_samples_iter_%03d.png' % (save_path, iters),
                    nrow=8,
                    normalize=True,
                )
