    ...
    for i in os.listdir(actual_dir):
        ...
        for k in range(path_size):
            if count <= int(path_size * (split_size[0])):
                shutil.move(os.path.join(path, os.listdir(path)[int(random_list[k])]), os.path.join(train_dir, i))
                count += 1
            # Moving data from actual_dir to test_dir
            elif count <= int(path_size * (split_size[0] + split_size[1])):
                shutil.move(os.path.join(path, os.listdir(path)[int(random_list[k])]), os.path.join(teste_dir, str(i)))
                count += 1
...
def make_dataset(args):
    ...
    classes = {0: 'ViseScrew', 1: 'Clamp', 2:'Nut', 3: 'Base', 4: 'All', 5: 'HeadPlate', 6: 'BallJoint', 7: 'Screw', 8: 'ClampScrew', 9: 'Slider'}
    ...
    if os.path.exists(data_root):
        ...
        for i in os.listdir(data_root):
            for index, classe in classes.items():
                if classe == i:
                    os.rename(os.path.join(data_root, i), os.path.join(data_root, str(index)))
        ...
        random_list = pd.read_csv('random_list.csv', sep=";", header=None).iloc[0].tolist()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_root', type=str, default=None, help='Path to the data root')
